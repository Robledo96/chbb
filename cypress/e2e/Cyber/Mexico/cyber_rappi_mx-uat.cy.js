   ...
   //
    ...
    it('Personal Details', () => {
        cy.fixture('locators').then((x) => {
            cy.get(x.input_name, { timeout: 30000 }).type(person.name)
                ...
                .get(x.input_id).click().type('ANML891018J47').wait(500)//'ANML891018J47'
    ...
    it('Edit', () => {
        cy.fixture('locators').then((x) => {
            cy.get(x.input_colonia, { timeout: 30000 }).click({ force: true })
                .get(x.colonia_option_text, { timeout: 5000 }).eq(0).click({ force: true })
                .wait(1000)
    ...
    it('Thankyou', () => {
        ...
        cy.get('.thank-you__policy-content__code').invoke('text').then(text => {
            let code = text + '\n'
            cy.writeFile('cypress/e2e/Cyber/policy_code-Cyber.txt', code, { flag: 'a+' })
        })
    })
})
