...
describe('Travel orientalbank PUERTO RICO (uat)', () => {
    //Page 1
    it('Visit', () => {
    ...
    it(' Number of Travelers ', () => {
        cy.fixture('locators').then((x) => {
            cy.get(x.select_placeholder, { timeout: 50000 }).click({ timeout: 50000 })
                .get(x.select_option).should('have.length.greaterThan', 0)
                .its('length').then(($length) => {
                    cy.log($length)
    ...
    it('Payment page', () => {
        cy.fixture('locators').then((x) => {
            cy.iframe(x.card_iframe).then($ => {
                cy.wrap($[0])
                    .find(x.input_card, { timeout: 6000 }).click()
                    .type(payment.visa_card_num_1, { delay: 60 })
                    .get(x.input_card_name, { timeout: 6000 }).type(payment.card_holder, { delay: 60 })
                    .get(x.input_expiry_date, { timeout: 6000 }).type(payment.expiration_date, { delay: 60 })
            })
            cy.iframe(x.cvv_iframe).then($iframes => {
                cy.wrap($iframes[0])
                    .find(x.input_cvv, { timeout: 6000 }).click()
